...
<div class=" container-fluid   container-fixed-lg bg-white">
    ...
    <div class="card card-transparent">
        <div class="card-body">
            <form class="form-horizontal" method="POST" action="{{ url('sujets/'.$topic->id.'/modifier') }}">
                        {{ csrf_field() }}
                        {{ method_field('POST') }}
                <div class="row clearfix">
                    <div class="col-md-12">
                        <div class="form-group {{ $errors->has('title') ? ' has-error' : '' }} form-group-default required" aria-required="true">
                            <label>Title</label>
                            <input type="text" class="form-control error" name="title" value="{{$topic->title}}" required autofocus aria-required="true"
                            aria-invalid="true">
                        </div>
                        @if ($errors->has('title'))
                    ...
                    <div class="col-md-12">
                            <div class="form-group {{ $errors->has('description') ? ' has-error' : '' }} form-group-default required" aria-required="true">
                                    ...
                                    <textarea class="form-control" name="description" id="" cols="221" rows="10">{{$topic->description}}</textarea>
                            </div>
