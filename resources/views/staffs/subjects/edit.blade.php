    ...
    <div class=" container-fluid   container-fixed-lg sm-p-l-0 sm-p-r-0">
        <div class="inner" style="transform: translateY(0px); opacity: 1;">
            ...
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">
                    <a href="{{url('sujets/'.$subject->topic->id.'/detail')}}" >{{$subject->topic->title}}</a>
                </li>
                <li class="breadcrumb-item">
...
<div class=" container-fluid   container-fixed-lg bg-white">
    ...
    <div class="card card-transparent">
        <div class="card-body">
            <form class="form-horizontal" method="POST" action="{{ url('sujets/'.$subject->topic->id.'/detail/'.$subject->id.'/modifier') }}">
                        {{ csrf_field() }}
                        {{ method_field('POST') }}
                <div class="row clearfix">
                    <div class="col-md-12">
                        <div class="form-group {{ $errors->has('title') ? ' has-error' : '' }} form-group-default required" aria-required="true">
                            <label>Title</label>
                            <input type="text" class="form-control error" name="title" value="{{$subject->title}}" required autofocus aria-required="true"
                            aria-invalid="true">
                        </div>
                        @if ($errors->has('title'))
                    ...
                    <div class="col-md-12">
                            <div class="form-group {{ $errors->has('description') ? ' has-error' : '' }} form-group-default required" aria-required="true">
                                    ...
                                    <textarea class="form-control" name="description" id="" cols="221" rows="10">{{$subject->description}}</textarea>
                            </div>
